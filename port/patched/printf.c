...
/*
 * X86_64 PORT: the %r/%R register-decode path fabricated a va_list by
 * casting &field — valid on the MIPS ABIs, where va_list is a simple
 * pointer, but not on x86-64 SysV, where va_list is a one-element struct
 * array.  This variadic springboard builds a genuine va_list instead.
 */
static void
errprintf_1arg(int where, char **buf, char *fmt, ...)
{
	va_list ap;
	...
	case 'R':
		...
		if (c == 'r' || b) {
			...
			for (rd = (struct reg_desc *)s; rd->rd_mask; rd++) {
				...
				if (rd->rd_format) {
					/* X86_64 PORT: real va_list (above) */
					errprintf_1arg(where, buf,
					    rd->rd_format, field);
					any = 1;
					if (rd->rd_values)
						output(where, buf, ':');
